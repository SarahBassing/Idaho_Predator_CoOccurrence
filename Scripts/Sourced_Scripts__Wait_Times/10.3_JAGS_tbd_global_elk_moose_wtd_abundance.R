  #'  ----------------------------------------
  #'  Wait time global (elk, moose, wtd) non-target model
  #'  
  ...
  #'  ----------------------------------------
  #'  Estimate effect of relative abundance of elk/moose/wtd and prey species diversity
  #'  index on the time that elapses before detecting a focal predator species.
  #'  -----------------------------------------

  cat(file = "./Outputs/tbd_global_elk_moose_wtd_abundance.txt", "
      model{
      ...
      for(i in 1:ntbd){
        ...
        log(tbd_mu[i]) <- alpha0 + beta.sppID[covs[i,2]] + beta.prey[1]*covs[i,5] + beta.prey[2]*covs[i,6] + beta.prey[3]*covs[i,7] + beta.div*covs[i,9] 
      }
      
      #'  Derived parameters
      #'  ------------------
      #'  Mean TBD of each species at average prey RAI and prey diversity
      for(spp in 1:nspp) {
        spp.tbd[spp] <- exp(alpha0 + beta.sppID[spp] + beta.prey[1]*0 + beta.prey[2]*0 + beta.prey[3]*0 + beta.div*0)
      }
      
      #'  Mean TBD per species across range of elk/moose/wtd relative abundance values and species diversity
      for(i in 1:100){
        for(spp in 1:nspp){
