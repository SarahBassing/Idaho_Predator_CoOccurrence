  #'  ----------------------------------------
  #'  Wait time prey RIA (elk, moose, wtd) model
  #'  
  ...
  #'  ----------------------------------------
  #'  Estimate effect of relative abundance of elk/moose/wtd on the time that 
  #'  elapses before detecting a focal predator species. 
  #'  -----------------------------------------
  
  cat(file = "./Outputs/tbd_elk_moose_wtd_abundance.txt", "
      model{
      ...
      for(i in 1:ntbd){
        ...
        log(tbd_mu[i]) <- alpha0 + beta.prey[1]*covs[i,5] + beta.prey[2]*covs[i,6] + beta.prey[3]*covs[i,7] 
      }
