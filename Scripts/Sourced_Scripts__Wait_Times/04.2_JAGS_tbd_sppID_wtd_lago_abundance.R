  #'  ----------------------------------------
  #'  Wait time species ID + prey RIA (wtd & lagomorphs) model
  #'  
  ...
  #'  abundance of wtd/lagomorph on the time that elapses before detecting a focal 
  #'  predator species.
  #'  -----------------------------------------
  
  cat(file = "./Outputs/tbd_sppID_wtd_lago_abundance.txt", "
      model{
      ...
      #'  Mean TBD per competitor across range of wtd/lagomorph relative abundance values
      for(i in 1:100){
        for(spp in 1:nspp){
