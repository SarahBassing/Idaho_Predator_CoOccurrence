  #'  ----------------------------------------
  #'  Wait time global (elk, moose, wtd) model
  #'  
  ...
  #'  relative abundance of elk/moose/wtd and prey species diversity index on the 
  #'  time that elapses before detecting a focal predator species.
  #'  -----------------------------------------
  
  cat(file = "./Outputs/tbd_global_sppID_X_div_elk_moose_wtd_abundance.txt", "
      model{
      ...
      for(i in 1:ntbd){
        ...
        log(tbd_mu[i]) <- alpha0 + beta.sppID[covs[i,2]] + beta.prey[1]*covs[i,5] + beta.prey[2]*covs[i,6] + beta.prey[3]*covs[i,7] + beta.div*covs[i,9] +
                          beta.interaction.elk[covs[i,2]]*covs[i,5] + beta.interaction.moose[covs[i,2]]*covs[i,6] + beta.interaction.wtd[covs[i,2]]*covs[i,7] + beta.interaction[covs[i,2]]*covs[i,9]
      }
