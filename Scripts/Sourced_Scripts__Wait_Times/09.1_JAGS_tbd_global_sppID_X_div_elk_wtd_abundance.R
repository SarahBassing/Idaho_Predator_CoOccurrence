  #'  ----------------------------------------
  #'  Wait time global (elk & wtd) model
  #'  
  ...
  #'  relative abundance of elk/wtd and prey species diversity index on the time 
  #'  that elapses before detecting a focal predator species.
  #'  -----------------------------------------
  
  cat(file = "./Outputs/tbd_global_sppID_X_div_elk_wtd_abundance.txt", "
      model{
      ...
      for(i in 1:ntbd){
        ...
        log(tbd_mu[i]) <- alpha0 + beta.sppID[covs[i,2]] + beta.prey[1]*covs[i,5] + beta.prey[2]*covs[i,7] + beta.div*covs[i,9] +
                          beta.interaction.elk[covs[i,2]]*covs[i,5] + beta.interaction.wtd[covs[i,2]]*covs[i,7] + beta.interaction[covs[i,2]]*covs[i,9]
      }
      ...
      for(spp in 1:nspp) {
        spp.tbd[spp] <- exp(alpha0 + beta.sppID[spp] + beta.prey[1]*0 + beta.prey[2]*0 + beta.div*0 +
                             beta.interaction.elk[spp]*0 + beta.interaction.wtd[spp]*0 + beta.interaction[spp]*0)
      }
      
      #'  Mean TBD per competitor across range of elk/wtd relative abundance values & prey diversity
      for(i in 1:100){
        for(spp in 1:nspp){
          spp.tbd.elk[i,spp] <- exp(alpha0 + beta.sppID[spp] + beta.prey[1]*newcovs[i,1] + beta.prey[2]*0 + beta.div*0 +
                                     beta.interaction.elk[spp]*newcovs[i,1] + beta.interaction.wtd[spp]*0 + beta.interaction[spp]*0)
          spp.tbd.wtd[i,spp] <- exp(alpha0 + beta.sppID[spp] + beta.prey[1]*0 + beta.prey[2]*newcovs[i,3] + beta.div*0 +
                                     beta.interaction.elk[spp]*0 + beta.interaction.wtd[spp]*newcovs[i,3] + beta.interaction[spp]*0)
          spp.tbd.div[i,spp] <- exp(alpha0 + beta.sppID[spp] + beta.prey[1]*0 + beta.prey[2]*0 + beta.div*newcovs[i,5] +
                                     beta.interaction.elk[spp]*0 + beta.interaction.wtd[spp]*0 + beta.interaction[spp]*newcovs[i,5])
        }
      }
