  #'  ----------------------------------------
  #'  Wait time global (wtd & lagomorph) non-target model
  #'  
  ...
  #'  ----------------------------------------
  #'  Estimate effect of relative abundance of wtd/lagomorph and prey species diversity
  #'  index on the time that elapses before detecting a focal predator species.
  #'  -----------------------------------------
  
  cat(file = "./Outputs/Time_btwn_Detections/tbd_global_wtd_lago_abundance.txt", "
      model{
      ...
      #'  Mean TBD per competitor across range of wtd/lagomorph relative abundance values & species diversity
      for(i in 1:100){
        for(spp in 1:nspp){
