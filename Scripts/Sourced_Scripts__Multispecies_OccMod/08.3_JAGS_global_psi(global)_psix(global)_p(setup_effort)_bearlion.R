  ...
  #'  ------------------------------------
  #'  Includes relative abundance indices for elk and white-tailed deer in addition 
  #'  to species diversity and basic habitat variables (elevation and percent forest 
  #'  cover), year, and camera setup on first-order psi. Elk, white-tailed deer 
  ...
  cat(file = './Outputs/08.3_JAGS_global_psi(global)_psix(global)_p(setup_effort)_bearlion.txt', "
      model{
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate indices: Intercept[1] + Setup[2] + Year[5] + Elevation[3] + Forest[4] + Elk[7] + White-tailed deer[9] + SppDiversity[6]
          psiSpp1[i] <- betaSpp1[1]*psi_cov[i,1] + betaSpp1[2]*psi_cov[i,2] + betaSpp1[3]*psi_cov[i,5] + betaSpp1[4]*psi_cov[i,3] + betaSpp1[5]*psi_cov[i,4] + betaSpp1[6]*psi_cov[i,7] + betaSpp1[7]*psi_cov[i,9] + betaSpp1[8]*psi_cov[i,6] 
          psiSpp2[i] <- betaSpp2[1]*psi_cov[i,1] + betaSpp2[2]*psi_cov[i,2] + betaSpp2[3]*psi_cov[i,5] + betaSpp2[4]*psi_cov[i,3] + betaSpp2[5]*psi_cov[i,4] + betaSpp2[6]*psi_cov[i,7] + betaSpp2[7]*psi_cov[i,9] + betaSpp2[8]*psi_cov[i,6] 
          
          #'  ...for state Spp12
          #'  Covariate indices: Spp12 = Intercept[1] + Setup[2] + Elk[7] + White-tailed deer[9] + SppDiversity[6]
          psiSpp12[i] <- psiSpp1[i] + psiSpp2[i] + betaSpp12[1]*psi_inxs_cov[i,1] + betaSpp12[2]*psi_inxs_cov[i,2] + betaSpp12[3]*psi_inxs_cov[i,7] + betaSpp12[4]*psi_inxs_cov[i,9] + betaSpp12[5]*psi_inxs_cov[i,6]    
