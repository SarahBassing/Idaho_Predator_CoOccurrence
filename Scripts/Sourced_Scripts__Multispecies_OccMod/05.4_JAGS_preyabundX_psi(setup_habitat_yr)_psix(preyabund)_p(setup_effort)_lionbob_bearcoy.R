#'  ------------------------------------
#'  Prey relative abundance model w/ interaction
#'  For lion-bobcat dyad
#'  
...
#'  ------------------------------------
#'  Includes relative abundance indices for elk, white-tailed deer, and lagomorphs 
#'  in addition to basic habitat variables (elevation and percent forest cover), 
#'  year, and camera setup on first-order psi. Elk, white-tailed deer, lagomorph 
...
cat(file = './Outputs/05.4_JAGS_preyabundX_psi(setup_habitat_yr)_psix(preyabund)_p(setup_effort)_lionbob_bearcoy.txt', "
      model{
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate order: Spp1 = Intercept[1] + Setup[2] + Year[3] + Forest[4] + Elevation[5] + TRI[10] + Elk[6] + White-tailed deer[8] 
          #'  Covariate order: Spp2 = Intercept[1] + Setup[2] + Year[3] + Forest[4] + Elevation[5] + TRI[10] + White-tailed deer[8] + Lagomorph[9] 
          psiSpp1[i] <- betaSpp1[1]*psi_cov[i,1] + betaSpp1[2]*psi_cov[i,2] + betaSpp1[3]*psi_cov[i,3] + betaSpp1[4]*psi_cov[i,4] + betaSpp1[5]*psi_cov[i,5] + betaSpp1[6]*psi_cov[i,10] + betaSpp1[7]*psi_cov[i,6] + betaSpp1[8]*psi_cov[i,8] 
          psiSpp2[i] <- betaSpp2[1]*psi_cov[i,1] + betaSpp2[2]*psi_cov[i,2] + betaSpp2[3]*psi_cov[i,3] + betaSpp2[4]*psi_cov[i,4] + betaSpp2[5]*psi_cov[i,5] + betaSpp2[6]*psi_cov[i,10] + betaSpp2[7]*psi_cov[i,8] + betaSpp2[8]*psi_cov[i,9] 
          
          #'  ...for state Spp12
          #'  Covariate order: Spp12 = Intercept[1] + Setup[2] + Elk[6] + White-tailed deer[8] + Lagomorph[9]
          psiSpp12[i] <- psiSpp1[i] + psiSpp2[i] + betaSpp12[1]*psi_inxs_cov[i,1] + betaSpp12[2]*psi_inxs_cov[i,2] + betaSpp12[3]*psi_inxs_cov[i,6] + betaSpp12[4]*psi_inxs_cov[i,8] + betaSpp12[5]*psi_inxs_cov[i,9]
