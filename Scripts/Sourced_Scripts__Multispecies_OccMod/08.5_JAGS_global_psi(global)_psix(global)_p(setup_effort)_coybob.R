  ...
  #'  and sampling effort on first-order rho. Allows species co-occurrence to be 
  #'  non-independent but assumes independent co-detection.
  #'  ------------------------------------
  
  cat(file = './Outputs/08.5_JAGS_global_psi(global)_psix(global)_p(setup_effort)_coybob.txt', "
      model{
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate inidices: Intercept[1] + Setup[2] + Year[5] + Elevation[3] + Forest[4] + White-tailed deer[9] + Lagomorph[10] + SppDiversity[6]
