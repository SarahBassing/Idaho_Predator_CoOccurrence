  ...
  cat(file = './Outputs/03.2_JAGS_preyabund_psi(setup_preyabund_yr)_p(setup_effort)_wolfcoy.txt', "
      model{
        ...
        #'  NOTE: number of priors & covariates for to betaSpp1 & betaSpp2 differ!
        for(wolf_psi in 2:9){                         
          betaSpp1[wolf_psi] ~ dnorm(0, 0.1)
        }
        for(coy_psi in 2:8){                          
          betaSpp2[coy_psi] ~ dnorm(0, 0.1)
        }
