  ...
  #'  percent forest cover), year, and camera setup on first-order psi. Elk, 
  #'  white-tailed deer, lagomorph abundance index and prey diversity on 
  #'  second-order psi. Setup and sampling effort on first-order rho. Allows species 
  #'  co-occurrence to be non-independent but assumes independent co-detection.
  #'  ------------------------------------
  
  cat(file = './Outputs/08.4_JAGS_global_psi(global)_psix(global)_p(setup_effort)_lionbob.txt', "
      model{
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate indices: Spp1 = Intercept[1] + Setup[2] + Year[5] + Elevation[3] + Forest[4] + Elk[7] + White-tailed deer[9] + SppDiversity[6]
          ...
          #'  ...for state Spp12
          #'  Covariate indices: Spp12 = Intercept[1] + Setup[2] + Elk[7] + White-tailed deer[9] + Lagomorph[10] + SppDiversity[6]
          psiSpp12[i] <- psiSpp1[i] + psiSpp2[i] + betaSpp12[1]*psi_inxs_cov[i,1] + betaSpp12[2]*psi_inxs_cov[i,2] + betaSpp12[3]*psi_inxs_cov[i,7] + betaSpp12[4]*psi_inxs_cov[i,9] + betaSpp12[5]*psi_inxs_cov[i,10] + betaSpp12[6]*psi_inxs_cov[i,6]
