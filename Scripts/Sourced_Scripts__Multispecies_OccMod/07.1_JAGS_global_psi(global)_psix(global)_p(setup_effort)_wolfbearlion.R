  ...
  cat(file = './Outputs/07.1_JAGS_global_psi(global)_psix(global)_p(setup_effort)_wolfbearlion.txt', "
      model{
        ...
        for(i in 1:nsites) {
          ...
          psiSpp12[i] <- psiSpp1[i] + psiSpp2[i] + betaSpp12[1]*psi_inxs_cov[i,1] + betaSpp12[2]*psi_inxs_cov[i,2] + betaSpp12[3]*psi_inxs_cov[i,6] + betaSpp12[4]*psi_inxs_cov[i,7] + betaSpp12[5]*psi_inxs_cov[i,8] 
