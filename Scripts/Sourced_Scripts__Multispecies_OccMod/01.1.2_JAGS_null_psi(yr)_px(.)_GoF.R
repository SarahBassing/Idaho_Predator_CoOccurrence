  #'  ---------------------------------
  #'  Null model with interaction on detection
  #'  
  ...
  #'  ---------------------------------
  #'  Includes only year effect on first-order psi to account for temporal  
  #'  correlation in occurrence related to re-sampling same site each year.
  #'  Assumes species occur independently of one another but co-detection is 
  #'  non-independent. Second half of script generates Chi-squared and Freeman-Tukey 
  #'  test statistics for Goodness-of-Fit test for detection probability submodel.
  #'  ---------------------------------
  
  cat(file = './Outputs/01.1.2_JAGS_null_psi(yr)_px(.)_GoF.txt', "
      model{
        ...
        #'  Assumes no second-order interactions by fixing to 0
        betaSpp12 <- 0
