  ...
  #'  ------------------------------------
  #'  Includes relative abundance indices for elk, moose, white-tailed deer, 
  #'  lagomorphs and a prey diversity index (Shannon's index), in addition to basic  
  #'  habitat variables (elevation and percent forest cover), year, and camera 
  #'  setup on first-order psi. Elk, moose, white-tailed deer, lagomorph relative 
  #'  abundance index and prey diversity on second-order psi. Setup and sampling 
  #'  effort on first-order rho. Allows species co-occurrence to be non-independent.
  #'  ------------------------------------
  
  cat(file = './Outputs/08.2_JAGS_global_psi(global)_psix(global)_p(setup_effort)_wolfcoy.txt', "
      model{
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate indices: Spp1 = Intercept[1] + Setup[2] + Year[5] + Elevation[3] + Forest[4] + Elk[7] + Moose[8] + White-tailed deer[9] + SppDiversity[6]
          #'  Covariate indices: Spp2 = Intercept[1] + Setup[2] + Year[5] + Elevation[3] + Forest[4] + White-tailed deer[9] + Lagomorph[10] + SppDiversity[6]
          psiSpp1[i] <- betaSpp1[1]*psi_cov[i,1] + betaSpp1[2]*psi_cov[i,2] + betaSpp1[3]*psi_cov[i,5] + betaSpp1[4]*psi_cov[i,3] + betaSpp1[5]*psi_cov[i,4] + betaSpp1[6]*psi_cov[i,7] + betaSpp1[7]*psi_cov[i,8] + betaSpp1[8]*psi_cov[i,9] + betaSpp1[9]*psi_cov[i,6]
          ...
          #'  ...for state Spp12
          #'  Covariate order: Spp12 = Intercept[1] + Setup[2] + Elk[7] + Moose[8] + White-tailed deer[9] + Lagomorph[10] + SppDiversity[6]
          psiSpp12[i] <- psiSpp1[i] + psiSpp2[i] + betaSpp12[1]*psi_inxs_cov[i,1] + betaSpp12[2]*psi_inxs_cov[i,2] + betaSpp12[3]*psi_inxs_cov[i,7] + betaSpp12[4]*psi_inxs_cov[i,8] + betaSpp12[5]*psi_inxs_cov[i,9] + betaSpp12[6]*psi_inxs_cov[i,10] + betaSpp12[7]*psi_inxs_cov[i,6] 
