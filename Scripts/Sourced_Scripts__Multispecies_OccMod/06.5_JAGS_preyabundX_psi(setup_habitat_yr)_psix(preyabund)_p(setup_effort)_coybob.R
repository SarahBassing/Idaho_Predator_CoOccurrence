  ...
  cat(file = './Outputs/06.5_JAGS_preyabundX_psi(setup_habitat_yr)_psix(preyabund)_p(setup_effort)_coybob.txt', "
      model{
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate indices: Intercept[1] + Setup[2] + Year[5] + Elevation[3] + Forest[4] + White-tailed deer[9] + Lagomorph[10]
          psiSpp1[i] <- betaSpp1[1]*psi_cov[i,1] + betaSpp1[2]*psi_cov[i,2] + betaSpp1[3]*psi_cov[i,5] + betaSpp1[4]*psi_cov[i,3] + betaSpp1[5]*psi_cov[i,4] + betaSpp1[6]*psi_cov[i,9] + betaSpp1[7]*psi_cov[i,10]
          ...
          #'  ...for state Spp12
          #'  Covariate indices: Spp12 = Intercept[1] + Setup[2] + White-tailed deer[9] + Lagomorph[10]
          psiSpp12[i] <- psiSpp1[i] + psiSpp2[i] + betaSpp12[1]*psi_inxs_cov[i,1] + betaSpp12[2]*psi_inxs_cov[i,2] + betaSpp12[3]*psi_inxs_cov[i,9] + betaSpp12[4]*psi_inxs_cov[i,10]  
