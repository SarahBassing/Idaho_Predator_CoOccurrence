  #'  ------------------------------------
  #'  Prey diversity model with interaction
  #'  
  ...
  cat(file = './Outputs/07_JAGS_preydivX_psi(setup_habitat_yr)_psix(preydiversity)_p(setup_effort).txt', "
      model{
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate indices: Intercept[1] + Setup[2] + Year[5] + Elevation[3] + Forest[4] + SppDiversity[6]
          ...
          #'  ...for state Spp12
          #'  Covariate order: Spp12 = Intercept[1] + Setup[2] + SppDiversity[6] 
          psiSpp12[i] <- psiSpp1[i] + psiSpp2[i] + betaSpp12[1]*psi_inxs_cov[i,1] + betaSpp12[2]*psi_inxs_cov[i,2] + betaSpp12[3]*psi_inxs_cov[i,6] 
