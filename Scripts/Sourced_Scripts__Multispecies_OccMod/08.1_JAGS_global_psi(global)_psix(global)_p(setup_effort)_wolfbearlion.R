  #'  ------------------------------------
  #'  Global model
  #'  For wolf-bear and wolf-lion dyads
  #'  
  ...
  #'  a prey diversity index (Shannon's index), in addition to basic habitat 
  #'  variables (elevation and percent forest cover), year, and camera setup on 
  #'  first-order psi. Elk, moose, and white-tailed deer relative abundance index
  #'  and prey diversity on second-order psi. Setup and sampling effort on 
  #'  first-order rho. Allows species co-occurrence to be non-independent.
  #'  ------------------------------------
  
  cat(file = './Outputs/08.1_JAGS_global_psi(global)_psix(global)_p(setup_effort)_wolfbearlion.txt', "
      model{
        ...
        for(i in 1:nsites) {
          ...
          #'  Covariate indices: Spp2 = Intercept[1] + Setup[2] + Year[5] + Elevation[3] + Forest[4] + Elk[7] + White-tailed deer[9] + SppDiversity[6]
