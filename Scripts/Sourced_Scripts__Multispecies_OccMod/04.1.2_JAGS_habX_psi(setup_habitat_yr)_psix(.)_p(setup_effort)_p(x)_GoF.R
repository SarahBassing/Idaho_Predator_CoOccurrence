  #'  -----------------------------------------
  #'  Habitat model with interaction on psi & p
  #'  
  ...
  #'  year, and camera setup on first-order psi. Setup and sampling effort on 
  #'  first-order rho. Allows species co-occurrence and co-detection to be non-independent.
  #'  Second half of script generates Chi-squared and Freeman-Tukey test statistics
  #'  for Goodness-of-Fit test for detection probability submodel.
  #'  ---------------------------------------
  
  cat(file = './Outputs/04.1.2_JAGS_habX_psi(setup_habitat_yr)_psix(.)_p(setup_effort)_p(x)_GoF.txt', "
      model{
